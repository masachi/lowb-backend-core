...
import io.github.masachi.plugins.audit.utils.JsqlParserUtil;
import io.github.masachi.plugins.audit.utils.MybatisUtils;
import io.github.masachi.plugins.audit.utils.ReflectionUtil;
...
public abstract class AbstractParser implements DataParser {
	...
	public List<Map<String, Object>> query(MybatisInvocation mybatisInvocation, BoundSql boundSql, SqlParserInfo sqlParserInfo, int idx) throws SQLException {
		...
		//如果是update 精确查询
		if(sqlParserInfo.getActionType() == SqlCommandType.UPDATE){
			sqlParserInfo.getColumns().forEach(item -> {
				item.getColumns().forEach((updateColumnItem) -> {
					Column columnItem = new Column();
					columnItem.setColumnName(updateColumnItem.getColumnName());

					updateColumns.add(columnItem);
				});
			});
			updateColumns.addAll(updateColumns);
		}else{
			//如果是delete，查询所有
